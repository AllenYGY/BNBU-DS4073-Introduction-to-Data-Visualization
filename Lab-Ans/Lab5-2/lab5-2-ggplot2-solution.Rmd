...
```{r}
# add text
p + annotate("text", x = 6, y = 5,label="test")+ annotate("rect",
             xmin = 5, xmax = 7,
             ymin = 4, ymax = 6,
             alpha = .3, fill = "yellow")
```
...
```{r}
# Write your answer here
p + theme(legend.text = element_text(colour = "red", size = 8, face = "italic"))
```
...
Given the default plot: 1. set the title to "Plot of Petal length and Sepal length", the name of x axis to "Sepal.length (cm)", the name of y axis to "Petal.length (cm)". 2. Change the color of plot title to blue and the font family to Playfair. 3. Change the appearance of axis text to dodgerblue and italic. (Hint: similar way as in modifying legend style.)

```{r}
# Default plot
p <- ggplot(iris, aes(Sepal.Length, Petal.Length, colour=Species)) + geom_point()
# write your answer here
p <- p + ggtitle("Plot of Petal length and Sepal length") +
  xlab("Sepal.length (cm)") + ylab("Petal.length (cm)")
p + theme(plot.title = element_text(colour = "blue", family = "Playfair"), axis.text = element_text(color = "dodgerblue", face = "italic"))
```
...
```{r}
# write your answer here
ggplot(iris, aes(Sepal.Length, Petal.Length, color = Species)) + geom_point(shape=1) + geom_smooth(method=lm)
```
...
```{r}
# write your answer here
ggplot(iris, aes(Sepal.Length, Petal.Length, color = Species)) + geom_point(shape=1) + geom_smooth(method=lm,se=FALSE)
```
...
model.lm<-lm(formula = mpg ~ hp, data = mtcars)
