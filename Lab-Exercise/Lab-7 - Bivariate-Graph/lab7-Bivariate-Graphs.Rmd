...
If you wanted to know which class of four-wheel drive car is the most common, how would you visualize the data?

```{r}
# write your answer here
# Filter the data for only four-wheel drive cars
four_wheel_drive <- mpg[mpg$drv == "4", ]

# Plot a bar chart for four-wheel drive cars
ggplot(four_wheel_drive, aes(x = class)) +
  geom_bar(fill = "steelblue") +
  labs(
    title = "Most Common Class of Four-Wheel Drive Cars",
    x = "Vehicle Class",
    y = "Count"
  ) +
  theme_minimal()
...
The current problem of above segmented bar chart is that the proportion of each drive type is not easy to read.

Let’s add percent labels to each segment!

First, we’ll create a summary dataset that has the necessary labels.
...
`geom_point` options can be used to change the \* `color` - point color \* `size` - point size \* `shape` - point shape \* `alpha` - point transparency. Transparency ranges from 0 (transparent) to 1 (opaque), and is a useful parameter when points overlap.
...
# write your answer here
# Load necessary libraries and data (from previous steps)
...
# The enhanced line plot with points and improved labeling
ggplot(plotdata,
       aes(x = year, 
           y = lifeExp)) +
...
# plot mean salaries to match the desired image with grid background
ggplot(plotdata, 
       aes(x = rank, 
           y = mean_salary)) +
  ...
  # 2. 在条形图顶部添加数据标签 (Mean Salary)
  geom_text(aes(label = dollar(mean_salary, accuracy = 1)), 
            vjust = -0.5, 
            size = 5,
            color = "black") +
...
Draw kernel density plot of Salaries taking `example 2.3 in lab6` as reference, where salary is x axes and title is 'Salary distribution by rank'.
...
A violin plot captures more a distribution’s shape than a boxplot, but does not indicate median or middle 50% of the data. A useful variation is to superimpose boxplots on violin plots.

$\color{red}{\text{In-class exercise}}$

Draw a boxplot on on top of a violin plot. The color of violins are 'cornflowerblue'.

For boxplot, set width to 0.15, color to 'orange', outlier.color to 'orange' and outlier.size to 2.

```{r}
# write your answer here
data(Salaries, package="carData")

library(ggplot2) # 确保导入 ggplot2
...
ggplot(Salaries, 
  ...
  # 1. 绘制小提琴图
  geom_violin(aes(fill = rank), # 按 rank 分组填充颜色
              fill = "cornflowerblue") + # 将小提琴的颜色设置为 'cornflowerblue'
  ...
  geom_boxplot(width = 0.15, 
               ...
               outlier.size = 2,
               alpha = 0) + # 设置 alpha=0 使箱子内部透明，只显示橙色边框
  
  # 3. 添加标题和轴标签
  labs(title = "Salary Distribution and Summary by Rank",
       subtitle = "Boxplot (Orange) on Violin Plot (Cornflowerblue)",
       x = "Academic Rank",
       y = "Salary") +
...
A popular method for comparing groups on a numeric variable is a mean plot with error bars. Error bars can represent standard deviations, standard errors of the means, or confidence intervals. In this section, we’ll plot means and standard errors.
...
Draw a plot like following.

Hint: group the Salaries by both rank and sex.
...
knitr::include_graphics("mean-chart.png")
```

### 3.6 Cleveland Dot Charts
...
ggplot(plotdata,
  ...
  labs(title = "2007 Life Expectancy for Asian Countries",
