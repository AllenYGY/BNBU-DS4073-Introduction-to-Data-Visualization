...
#  Try: plotdata2 <- Marriage %>% count(race) %>% arrange(desc(race))
...
Marriage %>%
  count(sign) %>%
  ggplot(aes(x = reorder(sign, n), y = n)) +
...
# write your answer here
library(ggplot2)
# Assuming the 'mosaicData' package is already loaded for the 'Marriage' dataset
library(mosaicData) 
...
ggplot(Marriage, aes(x = age)) +
  geom_histogram(fill = "cornflowerblue", 
                 color = "white",
                 bins = 20) + # Set the number of bins to 20
  labs(title="Participants by age (20 bins)",
       x = "Age")
...
### 2.2 Higtograms and Categories

$\color{red}{\text{In-class exercise}}$

Taking dataset `mpg` as example, this dataset contains fuel efficiency information for various vehicle models.

`displ:` Engine displacement (numeric type), in liters (L).

`drv:` Drive type (character type), such as front-wheel drive ("f"), rear-wheel drive ("r"), or four-wheel drive ("4").

You can look at distributions of different categories, by color:
...
ggplot(mpg, 
  ...
  labs(title = "Engine Displacement Distribution by Drive Type",
       x = "Engine Displacement (Liters)",
       fill = "Drive Type")

```

```{r figure3, echo=FALSE, out.width = '70%'}
...
# write your answer here
library(ggplot2)
# Assuming the 'mosaicData' package is loaded for the 'Marriage' dataset
library(mosaicData)

# plot the age distribution with gold dots and black borders
ggplot(Marriage, aes(x = age)) +
