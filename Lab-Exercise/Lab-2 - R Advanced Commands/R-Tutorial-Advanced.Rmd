...
# Set dim=c(2,4,3) and dim=c(2,3,4) and observe the data.
```

$\color{red}{\text{In class exercise}}$

```{r}
# Q1: create array with dim = c(2,4,3) and print the result
v = c(1:24)
result <- array(v,dim=c(2,4,3))
result

# create array with dim = c(2,3,4) and print the result
result <- array(v,dim=c(2,3,4))
result
...
print("Sort the columns of the matrix")
result
apply(result,2,sort)
...
result <- cbind(car_models, car_type, car_color, year)
print(result)
```
...
max_gdp <- max(data$gdp, na.rm = TRUE)
print(max_gdp)
# Q3: Get the country detail with max gdp
data[data$gdp == max(data$gdp, na.rm = TRUE), ]
# Q4: write the first 20 rows of data to a txt file "country20.txt".
subset_data <- head(data, 20)
write.table(subset_data, file = "country20.txt", row.names = FALSE, sep = "\t")
...
country20 <- read.csv("country20.csv", header = TRUE)
# Q7: What is the mean of GDP for the developing countries which have death rate higher than 10?

mean_gdp <- mean(data$gdp[data$develop == 1 & data$deathrate > 10],na.rm = TRUE)
#print("Mean GDP:")
print(mean_gdp)
```
...
sort.result <- sort(v,decreasing = FALSE)
print(sort.result)
```
