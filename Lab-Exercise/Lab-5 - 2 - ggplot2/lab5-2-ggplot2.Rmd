...
## 1.1 ggplot2 - Aesthetic

### 1.1.1 ggplot2 - Working with Axes

In this chapter, we will focus on the “Iris” dataset which is commonly used by data scientists.
...
```{r}
# add text
p + annotate("text", x = 6, y = 5, label = "sample (6,5)")
```
...
### 1.1.2 ggplot2 - Working with Legends
...
```{r}
# Write your answer here
p + theme(legend.text = element_text(color = "red", face = "italic"))
```
...
### 1.1.3 ggplot2 - Working with main titles and axis labels
...
# write your answer here
p + 
  ggtitle("Plot of Petal length and Sepal length") +
  xlab("Sepal.length (cm)") + 
  ylab("Petal.length (cm)") + 
  theme(
    plot.title = element_text(color = "blue", family = "serif"), 
    #  I don't have Playfair font in my system.
    axis.text = element_text(color = "dodgerblue", face = "italic")
  )
```

### 1.1.4 ggplot2 - Scatter Plots & Jitter Plots
...
```{r}
# Write your answer here
# Start with the previous plot showing points colored by Species
...
```{r}
# write your answer here
# Add regression lines without the shaded confidence region
ggplot(iris, aes(Sepal.Length, Petal.Length, color = Species)) + 
  geom_point(shape=1) +
...
## 1.2 ggplot2 - More than geom_point()
...
chic <- readr::read_csv("chicago-nmmaps-custom.csv")
```

To be able to visualize chic, we have to use ggplot2 library.
...
One can also combine several geometric layers. Please draw both points and lines in the plot.

```{r eval = FALSE}
# write your answer here
# Add both a point layer and a line layer
...
g + geom_point() + geom_line(color = "firebrick", linetype = "dotted", linewidth = 0.3)
